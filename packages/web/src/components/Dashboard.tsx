...
export function Dashboard({ initialSessions, stats, orchestratorId, projectName }: DashboardProps) {
  ...
  const { connected, reconnecting } = useSSEConnection();
  const isFlashing = useFlashAnimation([sessions.length, stats.workingSessions]);
  ...
  return (
    <div
      className={`px-8 py-7 transition-colors duration-300 ${isFlashing ? "bg-[rgba(59,130,246,0.05)]" : ""}`}
    >
